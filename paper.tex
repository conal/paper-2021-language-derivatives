...
\agda{Decs}
A \emph{predicate} \AB{P} is decidable when (the type) {\AB{P} \AB{x}} is decidable for all \AB{x} \stdlibCitep{Relation.Unary}:
\agda{Decidable}
...
Type isomorphisms (such as those in \figref{nu-delta-lemmas}) play an important role in decidability, namely that isomorphic types or predicates are equivalently decidable:\footnote{In fact, logical equivalence suffices for the results in this paper, but isomorphism enables variations such as generating all parses/proofs rather than just one.}
\agda{isomorphisms}
