...
\agda{𝒟[]⊙}
These facts suggest that \AB{𝒟} can be computed one list element at a time via a more specialized operation, as is indeed the case:\footnote{Repeated \AF{δ} application is expressed here via a standard left fold\out{ \stdlibCitep{Data.List}}:
\agda{foldl}\vspace{-2ex}
In \AF{𝒟foldl}, {\AB{X} \AS = \AB{A} \AF ✶ \AS → \AB B}.
}
\agda{δ}
%\vspace{-4ex}
\agda{𝒟foldl}
