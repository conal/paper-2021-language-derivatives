...
These levels avoid logical inconsistencies\out{ resulting from {\APT{Set} \AS : \APT{Set}}} but can be safely ignored in this paper (which is parametrized over \AB ℓ).
...
In the definition of {\AB P \AF ☆}, \AF{concat} flattens a list of lists into a single list via a right fold.
The predicate {\AD{All} \AB P} holds of a list \AB{ws} when \AB{P} holds for every element of \AB{ws}:\footnote{The data type constructors \AIC{[]} and \AIC{\_∷\_} are overloaded here, referring to both lists (as indices) and \AD{All}.}\\
\agda{foldr-concat}
\vspace{-1ex}
\agda{All}
These three definitions are provided in the Agda standard library \citep{agda-stdlib}.

Note that for a language \AB{P} and string \AB{w}, {\AB{P} \AB{w}} is the type of \emph{proofs} that {\AB{w} \AF ∈ \AB{P}}, in other words \emph{explanations} of membership, or \emph{parsings}.
(If the type {\AB{P} \AB{w}} is uninhabited, then {\AB{w} ∉ \AB{P}}.)
For instance, every proof of {\AB{w} \AF ∈ \AB{P} \AF ∪ \AB{Q}} contains a proof of {\AB{w} \AF ∈ \AB{P}} or of {\AB{w} \AF ∈ \AB{Q}} \emph{and} the knowledge of which one was chosen (with different proofs reflecting different choices).
Likewise, a proof of {w \AF ∈ \AB{P} \AF ⋆ \AB{Q}}\out{ (language concatenation)} includes the choice of strings \AB{u} and \AB{v}, a proof that {\AB{w} \AD ≡ \AB{u} \AF ⊙ \AB{v}}, and proofs that {\AB{u} \AF ∈ \AB{P}} and {\AB{v} \AF ∈ \AB{Q}}.
\rnc\source{Examples}
\figrefdef{language-examples}{Language examples}{\agda{examples}} contains some examples of languages (left) and corresponding membership proofs (right).%
\footnote{Some explanatory comments:
\begin{itemize}

\item Agda variable names can contain most characters other than spaces and parentheses, e.g., ``\AB{a∪b☆}''.
...
\item The \AF{inj₁} and \AF{inj₂} functions are left and right injections into a sum type, corresponding to the left and right introduction forms for logical disjunction.

%% \item The \AF{inj₁} and \AF{inj₂} functions are left and right injections/introductions into a sum/disjunction type.
...
\ifacm\else
%% \citep{Elliott2021-derivatives-repo}.
%% (\repoURL).
...
In order to convert languages into parsers, it will help to understand how language membership relates to the algebraic structure of lists, specifically the monoid formed by \AF{\_⊙\_} and \AIC{[]}.
Generalizing from languages to functions from lists to \emph{any} type, define
\agda{ν𝒟}
For languages, {\AF{𝒟} \AB{P} \AB{u}} is the set of \AB{u}-suffixes from \AB{P}, i.e., the strings \AB{v} such that {\AB{u} \AF ⊙ \AB{v} \AF ∈ \AB{P}}.

Since {\AB{u} \AF ⊙ \AIC{[]} \AD ≡ \AB{u}}, it follows that\footnote{Recall that Agda variable names can contain most characters other than spaces and parentheses, e.g., ``\AB{ν∘𝒟}'' as declared here.}\footnote{For functions \AB{f} and \AB{g}, {\AB{f} \AF{≗} \AB{g}} is extensional equality, i.e., {\AS ∀ \AB x \AS → \AB{f} \AB{x} \AD{≡} \AB{g} \AB{x}}.}
\agda{ν∘𝒟}
Moreover, the function \AF{𝒟} distributes over \AIC{[]} and \AF{\_⊙\_}:\footnote{In algebraic terms, argument-flipped \AF{𝒟} is a monoid homomorphism from lists to endofunctions.}
\agda{𝒟[]⊙}
These facts suggest that \AB{𝒟} can be computed one list element at a time via a more specialized operation, as is indeed the case:\footnote{Repeated \AF{δ} application is expressed here via a standard left fold (with {\AB{X} \AS = \AB{A} \AF ✶ \AS → \AB B} for \AF{𝒟foldl}):
\agda{foldl}
\vspace{-5ex}
}
\agda{δ}
...
Lemmas \AF{ν∘𝒟} and \AF{𝒟foldl} liberate this technique from the assumption that languages are represented \emph{symbolically}, by some form of grammar (e.g., regular or context-free), as will be exploited in \secref{Automatic Differentiation}.

In terms of automata theory, derived languages are states, with \AB{P} being the initial state and {\AF{𝒟} \AB{P} \AB{u}} the state reached from \AB{P} after consuming the string \AB{u}; \AF{δ} is the state transition function; and \AB{ν} is the set of accepting states\out{ \needcite{}}.
Lemmas \AF{ν∘𝒟} and \AF{𝒟foldl} capture the correctness of this state machine as a recognizer for the language \AB{P}.
...
For type \AB{A}, the type {\AD{Dec} \AB{A}} contains proof of \AB{A} or a proof of {\AF ¬ \AB{A}} (defined as usual to mean {\AB{A} \AS → \AD{⊥}}):\footnote{The Agda standard library contains a more efficient version of \AD{Dec} \stdlibCitep{Relation.Nullary}.}
\agda{Dec}
...
\item Recall that \AF{inj₁} and \AF{inj₂} are left and right injections into a sum type.
...
\agda{Decidable}
With these definitions, we can succinctly formulate the problem of decidable language recognition: given a language \AB{P}, construct a term of type {\AF{Decidable} \AB{P}}.
...
These lemmas and definitions cannot be applied \emph{automatically} in their present form, however, because languages are functions, as are \AF{ν} and \AF{δ}, and so are not subject to structural inspection.
An automatic solution would need some form of \emph{reflection} of language-constructing operations or of \AF{ν} and \AF{δ} as inspectable data.
...
The result is shown in \figrefdef{symbolic-api}{Regular expressions (inductive)}{\api} along with a trivial semantic function {\AF{⟦\_⟧}} (mapping any representation of type {\AF{Lang} \AB P} to the corresponding type-level language \AB{P}) and a less trivial, computable function  {\AF{⟦\_⟧‽}} that converts a symbolic language representation to a computable parser.
Note that the \AF{\_◂\_} operation from \secref{Decidability} has been reified as a new constructor alongside the more conventional language operators.
The reason \AF{\_◂\_} must be part of the inductive representation is the same as the other constructors, namely so that the core lemmas (\figref{nu-delta-lemmas}) translate into an implementation in terms of that representation.

Decidable parsing relies only on \AF{ν} and \AF{δ} (via {\AF{⟦\_⟧‽}}), which are defined in \figrefdef{symbolic-defs}{Symbolic differentiation (column-major/patterns)}{\agda{defs}}, as systematically derived from the lemmas of \figref{nu-delta-lemmas}.\out{\notefoot{Describe this derivation.}}
...
(Recall from \secref{Decidability} that correctness is defined as constructing a term of type {\AF{Decidable} \AF{P}} for a given language \AB{P}, as satisfied by the type of {\AF{⟦\_⟧‽}}.)
We could thus use \emph{any} definitions of \AF{ν} and \AF{δ} that type- and termination-check, although not many definitions would do so.
...
Dually, coinductive types (``codata'') describe infinitely large values and are processed recursively (as greatest fixed points) by copattern-matching clauses that compose results.
...
(As we will see in \secref{Transporting Properties from Specification to Implementations}, however, the simple relationship to the specification in \secref{Decomposing Languages} allows many trivial correctness proofs, needing neither introduction or coinduction.)
See \citet{Gordon1995:coinduction} for a tutorial on theory and techniques of coinduction.

Tries in general represent functions, with each trie datum corresponding to domain value.
...
Decidable recognition again relies only on \AF{ν} and \AF{δ}, which are defined for each language operation in \figrefdef{automatic-defs}{Automatic differentiation (row-major/copatterns)}{\agda{defs}} (though with a problem to be noted and fixed in the next paragraph).
...
The proof of correctness (defined by {\AF{⟦\_⟧}} and {\AF{⟦\_⟧‽}}, defined as before) is \emph{almost} accomplished by type- and termination-checking, but a technical problem arises.
...
\agda{ʻ𝒟}
Equivalence follows from lemmas \AF{ν∘𝒟} and \AF{𝒟foldl} of \secref{Decomposing Languages}:
\agda{𝒟′≡ʻ𝒟}
The trie representation exploits this sharing potential by weaving \AF{ν} and \AF{δ} into a single structure based on common prefixes.
...
Within the second category, \AF{𝟏}, \AF{\_⋆\_}, and \AF{\_☆} can apply to any monoid, while \AF{`} is specific to lists.
...
\end{minipage}
\hfill\;
}, which adds language complement (\AF ∁).\footnote{The list-specific operations \AF{\_⊙\_}, \AIC{[]}, and \AF{concat} used in \figref{Lang-ops} have been generalized to monoid operations {\AF{\_∙\_} and \AF{ε}}.
The \emph{definitions} of \AF{𝟏}, \AF{\_⋆\_}, and \AF{\_☆} do not require the monoid properties, but their properties will (\secref{Predicate Algebra}).
}

\rnc\source{Calculus}
The \AF{ν} and \AF{δ} lemmas in \figref{nu-delta-lemmas} for codomain (but not domain) operations are easily generalized as shown in \figrefdef{nu-delta-codomain-lemmas}{Properties of \AF{ν} and \AF{δ} for predicate codomain operations}{\setstretch{1.5}\agda{νδ-codomain}}, all proved automatically by normalization.
...
\end{code}
Referring to the type and operations of \figreftwo{Lang-ops}{symbolic-api} (for the inductive language representation) via the module prefixes ``{◇.}\hspace{0.05em}'' and ``{□.}\hspace{0.05em}'' respectively, define \agda{wrapped-Lang}
...
Wrapped union can then be defined simply as {\AF{\_∪\_} \AS = \AF{inj₂} \AIC{□.\_∪\_}} and likewise for the other operations.
...
For the language implementations of \secref{From Languages to Parsing}, this anchoring is guaranteed by the types of \AF{⟦\_⟧} and \AF{⟦\_⟧‽} in \figreftwo{symbolic-api}{sized-automatic-api}.
...
The difference is mainly visible in the \AF ν and \AF δ lemmas for {\AB P ⋆ \AB Q} and {\AB P ☆} in \figref{nu-delta-lemmas}.
