...
A simple characterization of languages (and indeed functions from lists to any type) captures the essential idea behind language ``differentiation'' (and integration) as used for recognizing languages and leads to a collection of lemmas about type-level predicates.
...
\agda{Lang}
\figrefdef{Lang-ops}{Languages as type-level predicates}{\agda{Lang-ops}} gives definitions of the usual language operations, including language union and intersection ({\AB{P} \AF ∪ \AB{Q}} and {\AB{P} \AF ∩ \AB{Q}}) with their identities (the empty language \AF ∅ and universal language \AF 𝒰), language concatenation ({\AB{P} \AF ⋆ \AB{Q}}) and its identity (\AF 𝟏, containing only the empty string), single-character languages ({\AF ` \AB{c}}), Kleene star ({\AB{P} \AF ☆}), and ``scalar multiplication'' (\mbox{\AB{s} \AF · \AB{P}}, which will prove useful later).
...
The specific embodiment of Curry-Howard in \figref{Lang-ops} and throughout this paper is the language Agda \citep{Norell2009,BoveEtAl2009Agda}, which is both programming language and proof assistant founded on Martin-Löf's intuitionistic type theory \citep{MartinLöf1984intuitionistic}.
Specifically,
\begin{itemize}
...
\item For a function {\AB F \AS : \AB A \AS → \APT{Set}}, the types {\AF ∃ \AB F}  and {\AF ∀ \AB x \AS → \AB F \AB x} represent existential and universal quantification as \emph{dependent} product and function types.
...
As shown in the rest of this paper, however, type-level predicates serve as a simple, precise, and non-operational basis, with compelling and correct implementations following easily and systematically as corollaries.
...
\item
  A duality is revealed between regular expressions and tries in the context of language differentiation.
...
\item
  The vocabulary and reasoning of languages is generalized to predicates over arbitrary types, and then re-specialized to the usual vocabulary.
  This generalization consists of two generally useful covariant applicative functors on functions (the usual one on functions-from-X and another on functions-to-Set).
  Properties are proved in terms of this more general vocabulary.
\item
  Properties are defined first at the level of predicates, making both easy to understand and prove, thanks to elimination of operational details.
  These properties are then transported effortlessly to the computable parsing implementations.
\end{itemize}
The source code for this paper is freely available and contains fully verified Agda code.
All type signatures and code in the paper (including all lemmas and algorithms whether shown in the paper or elided) are generated by the Agda compiler with no postulates.
...
\agda{νδ}
In terms of languages, {\AF{δ} \AB{P} \AB{a}} comprises all strings \AB{w} such that {\AB{a} \AIC ∷ \AB{w}} is in \AB{P}, i.e., the tails of strings in \AB{P} that start with \AB{a}.
...
\item {\AS λ \AS{()}} is the unique function from \AD ⊥ to any type (here also {\AD ⊥}).
...
\item Given two functions {\AB f \AS : \AB A \AS → \AB C} and {\AB g \AS : \AB B \AS → C}, the function {\AF [ \AB f \AF , \AB g \AF ] \AS : \AB A \AF ⊎ \AB B \AS → \AB C} maps {\AIC{inj₁} \AB a} to {\AB f \AB a} and {\AIC{inj₂} \AB b} to {\AB g \AB b}.
In this use, {\AB C} is \AD ⊥, since we are proving a negation.

\item The \AF{\_✶‽} definition reflects the fact that the empty list exists for every element type (even \AF ⊥).
...
Decidable recognition (via {\AF{⟦\_⟧}}) relies only on \AF{ν} and \AF{δ}, which are defined in \figrefdef{symbolic-defs}{Symbolic differentiation (column-major/patterns)}{\agda{defs}}, as systematically derived from the lemmas of \figref{nu-delta-lemmas}.\out{\notefoot{Describe this derivation.}}
...
This time, reflect \AF{ν} and \AF{δ} into a \emph{coinductive} data type of tries, while redefining the language-building vocabulary as functions on that data type defined by \emph{copatterns} \citep{AbelPientka2016}.\footnote{The classic trie (``prefix tree'') data structure was introduced by Thue in 1912 to represent sets of strings \citep[Section 6.3]{Knuth1998ACP3}, later generalized to arbitrary (regular) algebraic data types \citep{Connelly1995GenTrie}, and then from sets to functions \citep{Hinze2000GGT}.}
...
\end{code}
Referring to the type and operations of \secref{Specifying Languages} and \figref{symbolic-api} via the module prefixes ``{◬.}\hspace{0.05em}'' and ``{▣.}\hspace{0.05em}'' respectively, define \agda{wrapped-Lang}
...
\figrefdef{inj}{Existential wrappers}{\mathindent0ex\agda{inj}}, again with inferable components given explicitly for clarity.
...
(Similarly, for the decidable type constructors in \figref{compositional-dec}, correctness is guaranteed by the types of \AIC{yes} and \AIC{no} in the definition of ‌\ARe{Dec} in \secref{Decidability}.)
...
They prove a consistency property about \AF{δ}, but apparently not about \AF{ν}, and they note their method's ``inability to prove the non-membership of some string in a given language''.
...
%% requiring more careful construction and reading than the specification given in \figref{Lang-ops} and used in the implementations in \secreftwo{Symbolic Differentiation}{Automatic Differentiation}

Of the previous work involving language derivatives, none appear to use the essential, non-inductive, nearly-trivial definition in terms of functions of lists from \secref{Decomposing Languages}, although \citet{Abel2016} did indeed mention this definition informally as motivation for the more complex, coinductively defined operations on tries.
