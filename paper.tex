...
\ifacm
\documentclass[acmsmall,screen,timestamp,anonymous,review]{acmart}
\else
...
\item
  ...
  These properties are then transported to the computable parsing implementations (\secref{Transporting Properties}) with no additional proofs needed.
\end{itemize}
...
Since {\AB{u} \AF ⊙ \AIC{[]} \AD ≡ \AB{u}}, it follows that\footnote{Agda variable names can contain most characters other than spaces and parentheses, e.g., ``\AB{ν∘𝒟}'' as declared here.}\footnote{For functions \AB{f} and \AB{g}, {\AB{f} \AF{≗} \AB{g}} is extensional equality, i.e., {\AS ∀ \AB x \AS → \AB{f} \AB{x} \AD{≡} \AB{g} \AB{x}}.}
\agda{ν∘𝒟}
...
There has also been some work using type theory to capture languages as type-level predicates much as in \secref{Specifying Languages}:
\begin{itemize}
