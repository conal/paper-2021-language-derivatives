...
\rnc\source{Calculus}
The language representations in \secreftwo{Symbolic Differentiation}{Automatic Differentiation} adapt easily to the generalized codomain operations (\AF{pureᵀ}, \AF{mapᵀ}, and \AF{mapᵀ₂}) resulting in broader coverage with less code.
The relevant \AF{ν} and \AF{δ} lemmas in \figref{nu-delta-lemmas} are easily generalized as shown in \figrefdef{nu-delta-codomain-lemmas}{Properties of \AF{ν} and \AF{δ} for predicate codomain operations}{\agda{νδ-codomain}} and are all proved automatically by normalization.
