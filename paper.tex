...
\agda{𝒟[]⊙}
These facts suggest that \AB{𝒟} can be computed one list element at a time via a more specialized operation, as is indeed the case:\footnote{Repeated \AF{δ} application is expressed here via a standard left fold (with {\AB{X} \AS = \AB{A} \AF ✶ \AS → \AB B} in \AF{𝒟foldl}):
\agda{foldl}
}
\agda{δ}
%\vspace{-4ex}
...
Lemmas \AF{ν∘𝒟} and \AF{𝒟foldl} liberate this technique from the assumption that languages are represented \emph{symbolically}, by some form of grammar (e.g., regular or context-free), as we will exploit in \secref{Automatic Differentiation}.
...
As with all lemmas in this paper shown with signatures only, full proofs are in the paper's source code and are formally verified by the Agda compiler (including during typesetting).
...
For type \AB{A}, the type {\AF{Dec} \AB{A}} contains proof of \AB{A} or a proof of {\AF ¬ \AB{A}} (defined as usual to mean {\AB{A} \AS → \AD{⊥}}):\footnote{This simple \AF{Dec} definition been superceded by a more efficient version \stdlibCitep{Relation.Nullary}.}
\agda{Dec}
...
This latter option is often much more efficient than the former, since it easily avoids a good deal of redundant computation due to significant amounts of duplicated work between a function and its derivative.
...
\rnc\source{Calculus}
The \AF{ν} and \AF{δ} lemmas for codomain (but not domain) operations in \figref{nu-delta-lemmas} are easily generalized as shown in \figrefdef{nu-delta-codomain-lemmas}{Properties of \AF{ν} and \AF{δ} for predicate codomain operations}{\setstretch{1.5}\agda{νδ-codomain}} and are all proved automatically by normalization.
