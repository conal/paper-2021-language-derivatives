...
\end{itemize}
Although many proofs are elided below, the source code for this paper is freely available\footnote{\protect{\repoURL}} and contains fully verified Agda code with no postulates.
...
For convenience, use the same names as in \secref{Specifying Languages} for these new counterparts, and refer to the original versions via the module prefix ``{◇.}\hspace{0.05em}''.
The result is shown in \figrefdef{symbolic-api}{Regular expressions (inductive)}{\api} along with a computable function {\AF{⟦\_⟧‽}} that converts a symbolic language representation to a computable parser.
...
The meaning of any {\AB r \AK{:} \AF{Lang} \AB P} is defined simply as \AB{P}, ignoring the representation \AB{r} itself\out{ (beyond its type)}:
\agda{semantics}
