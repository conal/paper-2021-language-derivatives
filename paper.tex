...
Lemma \AF{ν∘foldlδ} liberates this technique from the assumption that languages are represented \emph{symbolically}, by some form of grammar (e.g., regular or context-free), inviting other representations as we will see in \secref{Automatic}.
...
\agda{Dec}
For compositionality, we will use higher-order counterparts lifting decidability of types to decidability of constructions on those types, as shown in \figrefdef{compositional-dec}{Compositional decidability}{\agda{compositional-dec}}.

A \emph{language} (or any predicate) \AB{P} is decidable when (the type) {\AB{P} \AB{x}} is decidable for all \AB{x} \stdlibCitep{Relation.Unary}:
\agda{Decidable}
Now we can formulate the problem of decidable language recognition: given a language \AB{P}, construct a term of type {\AF{Decidable} \AB{P}}.
...
The lemmas in \figref{nu-delta-lemmas} tell us how to decompose languages defined in terms of the vocabulary from \figref{Lang-ops}, and the definitions in \figref{compositional-dec} tell us how compute inhabitation of the resulting types, resulting in decidable parsing.
...
The latter solution involves re-interpreting the construction vocabulary for functions to construct not just the usual functions, but also their derivatives \needcite{}.
...
In the ``symbolic differentiation'' style, we reflect the language-building vocabulary of \figref{Lang-ops} into an inductive data type (of modestly extended regular expressions), while re-defining \AF{ν} and \AF{δ} as functions on that data type.
For convenience, use the same names as in \secref{Specifying Languages} for these new counterparts, while referring to the original versions via the prefix ``{◬.}'' (module-specific names).
...
Decidable recognition (via {\AF{⟦\_⟧}}) relies only on \AF{ν} and \AF{δ}, which are defined in \figrefdef{symbolic-defs}{Symbolic differentiation (column-major/patterns)}{\agda{defs}}.
These definitions are meant to be read in ``column-major'' order, i.e., each column is one function definition.
The \AF{Lang} representation is inductive, and definitions are by pattern matching.
Correctness is guaranteed by the types of \AF{ν} and \AF{δ}, so the correctness proof here is accomplished by type-checking.
...
This time, we reflect \AF{ν} and \AF{δ} into a \emph{coinductive} data type (of tries \needcite{}), while redefining the language-building vocabulary as functions on that data type.
Again, we will use the same names as in \secref{Specifying Languages}, referring to the original versions via the prefix ``{◬.}''.
...
These definitions are meant to be read in ``row-major'' order, i.e., each row is one function definition.
The \AF{Lang} representation is coinductive, and definitions are by \emph{copattern} matching \citep{AbelPientka2016}.
...
The \AF{δ} clause for {\AB{p} \AF ⋆ \AB{q}} does not satisfy Agda's termination checker, which cannot see that the {\AF{δ} \AB{p} \AB{a}} argument in the recursive use of \AF{\_⋆\_} is in some sense smaller than \AB{p}.
Fortunately, this issue was already solved by Andreas \citet{Abel2016}, also in the setting of trie-based language recognition, by using sized types \citep{Abel2008, AbelPientka2016}.
\rnc\source{SizedAutomatic}
This solution needs only give \AF{Lang} (now tries) an index {\AB{i} \AK : \APT{Size}} corresponding to the maximum depth to which a trie can be searched, or equivalently the longest string that can be matched\out{ (via {\AF{⟦\_⟧}})}.
...
Decidable recognition is defined exactly as with unsized tries (\figref{automatic-defs}), but this time the compiler successfully proves \emph{total} correctness (including termination).
