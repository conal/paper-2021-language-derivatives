...
In the ``symbolic'' style, we reflect the language-building vocabulary of \figref{Lang-ops} into an inductive data type, while re-defining \AF{ν} and \AF{δ} as functions on that data type.
...
The result is shown in \figrefdef{symbolic-api}{Regular expressions}{\agda{api}} along with a semantic function {\AF{⟦\_⟧}} that converts a symbolic language representation to decidable form.
...
These definitions are meant to be read in ``column-major'' order, i.e., the left column is one definition (of \AF{ν}), and the right column is another (of \AF{δ}).
