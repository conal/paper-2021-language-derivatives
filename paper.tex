...
\item
  A duality is revealed between regular expressions and tries (prefix trees) in the context of language differentiation (\secreftwo{Symbolic Differentiation}{Automatic Differentiation}).
...
Since the decidable language implementations in \secreftwo{Symbolic Differentiation}{Automatic Differentiation} are corollaries of \AF{ν} and \AF{δ} lemmas, those implementations adapt easily to the generalized codomain operations (\AF{pureᵀ}, \AF{mapᵀ}, and \AF{mapᵀ₂}), resulting in a somewhat smaller implementation and broader coverage.
...
We have seen that type-level languages have useful and illuminating algebraic properties and that the decidable implementations in \secreftwo{Symbolic Differentiation}{Automatic Differentiation} are tightly connected to languages.
...
For the language implementations of \secreftwo{Symbolic Differentiation}{Automatic Differentiation}, this anchoring is guaranteed by the type of \AF{⟦\_⟧} in \figreftwo{symbolic-api}{sized-automatic-api}.
